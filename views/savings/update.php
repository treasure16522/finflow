...
$this->title = 'Update Savings Goal: ' . $model->name;
$icons = ['🚨', '✈️', '🏠', '🎓', '🚗', '💍', '🎯', '🏦', '💻', '📱', '🎮', '📚', '🏥', '🛒', '⚽'];
?>

<div class="card" style="max-width: 600px; margin: 0 auto;">
    <div class="card-title">
        <span>✏️</span> Update Savings Goal
    </div>
    ...
    <div class="form-group full">
        <?= $form->field($model, 'name')->textInput(['placeholder' => 'Goal name']) ?>
    </div>
        ...
        <div style="display: flex; gap: 8px; flex-wrap: wrap; margin-top: 5px;">
            <?php foreach ($icons as $icon): ?>
                <label class="icon-radio" style="cursor: pointer; padding: 10px 15px; background: var(--bg3); border: 1px solid var(--border); border-radius: 8px; transition: all 0.2s;">
                    <input type="radio" name="SavingsGoal[icon]" value="<?= $icon ?>" style="display: none;" <?= $model->icon == $icon ? 'checked' : '' ?>>
    ...
    <div class="form-group">
        ...
        <input type="month" class="form-control" name="SavingsGoal[deadline]" 
               value="<?= $model->deadline ? date('Y-m', strtotime($model->deadline)) : '' ?>">
    </div>
    ...
    <div class="form-group full" style="display: flex; gap: 10px; justify-content: flex-end; margin-top: 20px;">
        ...
        <?= Html::submitButton('💾 Update Goal', ['class' => 'btn btn-primary']) ?>
    </div>
...
<script>
// Same icon selection script as create view
